  ...
  📁 generated/
    📁 prisma/
      ...
      📁 models/
        ...
        📄 User.ts
  📁 logs/
    📄 combined-2026-05-05.log
    📄 combined-2026-05-06.log
    📄 debug-2026-05-05.log
    📄 debug-2026-05-06.log
    📄 error-2026-05-05.log
    📄 error-2026-05-06.log
    📄 exceptions.log
    📄 rejections.log
  📁 prisma/
    📄 schema.prisma
...
### 📄 `logs\.07f7687b126ad7c5eca9e3f7aaf1d1b53a8df048-audit.json`
```json
{
    "keep": {
        "days": true,
    ...
    "auditLog": "logs\\.07f7687b126ad7c5eca9e3f7aaf1d1b53a8df048-audit.json",
    "files": [
        {
            "date": 1777988689823,
            "name": "logs\\combined-2026-05-05.log",
            "hash": "d53bec70cfaece3abbde710fe541dbef44b4dbc7efebe6b0e9d2f07c9fde4d3a"
        },
        {
            "date": 1778049218185,
            "name": "logs\\combined-2026-05-06.log",
            "hash": "796a8f9fad00025935ca5cf98abfd885bad30ab39fbb3cfca7f7d5bea9b78110"
        }
    ],
    "hashType": "sha256"
}
...
### 📄 `logs\.19234d63a085bf4f286d3d9ca0cc42c995c142ed-audit.json`
```json
{
    "keep": {
        "days": false,
    ...
    "auditLog": "logs\\.19234d63a085bf4f286d3d9ca0cc42c995c142ed-audit.json",
    "files": [
        {
            "date": 1777988689826,
            "name": "logs\\debug-2026-05-05.log",
            "hash": "f0cbc5be02f230264fa09fffc7073bdf68edaa9061f33f85b73fb005063c3c9f"
        },
        {
            "date": 1778049218187,
            "name": "logs\\debug-2026-05-06.log",
            "hash": "7b85502ff53ad6e2675bb8ef9e2067123c89e6d2bc2f1744d521bf176152ada7"
        }
    ],
    "hashType": "sha256"
}
...
### 📄 `logs\.1d2a7d70f4f9b6800fcd7ef60d22623acbcb7932-audit.json`
```json
{
    "keep": {
        "days": false,
    ...
    "auditLog": "logs\\.1d2a7d70f4f9b6800fcd7ef60d22623acbcb7932-audit.json",
    "files": [
        {
            "date": 1777988689821,
            "name": "logs\\error-2026-05-05.log",
            "hash": "eedd164ef64b7eb9468a9b76ca148704081290a350c60f2fb5382d2414cd2ca0"
        },
        {
            "date": 1778049218183,
            "name": "logs\\error-2026-05-06.log",
            "hash": "f5d0dd0d2439c14184ff63ee600fc0dd552176da7b824a9cbebf585d7027aca0"
        }
    ],
    "hashType": "sha256"
}
...
import { setIoInstance, fetchStatsInternal, emitStatsUpdate } from './services/statsService.js';
...
// Устанавливаем экземпляр io в statsService
setIoInstance(io);
app.set('io', io);
...
export const login = asyncHandler(async (req: any, res: Response) => {
  ...
  if (!result.success) {
    if (result.userBlocked) {
      return res.status(403).json({ error: 'Ваш аккаунт заблокирован', code: 'USER_BLOCKED' });
    }
    if (result.lockType === 'password') {
      return res.status(429).json({ error: 'Аккаунт заблокирован из-за частых ошибок', timeLeft: result.timeLeft, lockType: 'password' });
    }
    if (result.lockType === '2FA') {
      return res.status(429).json({ error: 'Аккаунт заблокирован из-за ошибок 2FA', timeLeft: result.timeLeft, lockType: '2FA' });
    }
    if (result.requires2FA) {
      return res.status(200).json({
        status: '2FA_REQUIRED',
        message: 'Требуется подтверждение входа (SMS)',
        data: { userId: result.userId, email: result.email, requires2FA: true },
      });
    }
    if (result.attemptsLeft !== undefined) {
      return res.status(401).json({ error: 'Неверный email или пароль', attemptsLeft: result.attemptsLeft });
    }
...
export const verify2FACode = asyncHandler(async (req: any, res: Response) => {
  ...
  if (!result.success) {
    if (result.locked) {
      return res.status(429).json({ error: 'Аккаунт заблокирован после неудачных попыток.', timeLeft: result.timeLeft, lockType: '2FA' });
    }
    if (result.attemptsLeft !== undefined) {
      return res.status(401).json({ error: 'Неверный код', attemptsLeft: result.attemptsLeft });
    }
...
export const logout = asyncHandler(async (req: any, res: Response) => {
  ...
  await logoutUser(userId, io, req.logMeta);
...
export const sendMessage = asyncHandler(async (req: any, res: Response) => {
  ...
  const { text } = req.body;
  const io = req.app.get('io');
  const message = await sendMessageService(Number(projectId), text, req.user.id, io, req.logMeta);
...
export const markAsRead = asyncHandler(async (req: any, res: Response) => {
  const { projectId } = req.params;
  const io = req.app.get('io');
  const result = await markMessagesAsReadService(Number(projectId), req.user.id, io, req.logMeta);
  res.json(result);
});
```
...
export const createProject = asyncHandler(async (req: any, res: Response) => {
  const io = req.app.get('io');
  const newProject = await createProjectService(req.body, req.user.id, io, req.logMeta);
...
export const updateProject = asyncHandler(async (req: any, res: Response) => {
  const { id } = req.params;
  const io = req.app.get('io');
  const updatedProject = await updateProjectService(Number(id), req.body, req.user.id, req.user.role, io, req.logMeta);
...
export const updateProjectStatus = asyncHandler(async (req: any, res: Response) => {
  ...
  const io = req.app.get('io');
  const updatedProject = await updateProjectStatusService(Number(id), status, req.user.id, req.user.role, io, req.logMeta);
...
export const getUsers = asyncHandler(async (req: any, res: Response) => {
  ...
  const io = req.app.get('io');
  const result = await getUsersList({ page: Number(page), limit: Number(limit), search: String(search), role: String(role), io });
...
export const toggleBlock = asyncHandler(async (req: any, res: Response) => {
  const { id } = req.params;
  const io = req.app.get('io');
  const { message, isBlocked } = await toggleBlockUser(Number(id), req.user.id, io, req.logMeta);
...
import { emitStatsUpdate, emitUserLockStatus } from './statsService.js';
import { AppError } from '../utils/AppError.js';
import logger from '../utils/logger.js';
...
export const logoutUser = async (userId: number | undefined, io: any, logMeta?: any) => {
  if (userId) {
...
export const getProjectMessages = async (projectId: number, userId: number, userRole: string, logMeta?: any) => {
  ...
  if (userRole !== 'MANAGER' && project.partnerId !== userId) {
    logger.warn('Unauthorized chat access attempt', { projectId, userId, ...logMeta });
...
export const sendMessage = async (projectId: number, text: string, senderId: number, io: any, logMeta?: any) => {
...
export const markMessagesAsRead = async (projectId: number, userId: number, io: any, logMeta?: any) => {
  ...
  if (io && updateResult.count > 0) {
    for (const sender of senders) {
      io.to(`user_${sender.senderId}`).emit('messages_read', { projectId, readerId: userId });
    }
...
import { emitStatsUpdate } from './statsService.js';
import { AppError } from '../utils/AppError.js';
import logger from '../utils/logger.js';

export const createProject = async (data: any, userId: number, io: any, logMeta?: any) => {
  ...
  if (existingProject) {
    logger.warn('Attempt to create duplicate project', { customerInn, userId, ...logMeta });
...
export const updateProject = async (id: number, data: any, userId: number, userRole: string, io: any, logMeta?: any) => {
  const project = await prisma.project.findUnique({ where: { id } });
  if (!project) throw new AppError(404, 'Проект не найден');
  if (project.partnerId !== userId && userRole !== 'MANAGER') {
    logger.warn('Unauthorized project update attempt', { projectId: id, userId, ...logMeta });
...
export const updateProjectStatus = async (id: number, status: string, userId: number, userRole: string, io: any, logMeta?: any) => {
  if (userRole !== 'MANAGER' && userRole !== 'ADMIN') {
    logger.warn('Unauthorized status change attempt', { projectId: id, userId, ...logMeta });
    throw new AppError(403, 'Недостаточно прав');
  }
  const project = await prisma.project.findUnique({ where: { id } });
  if (!project) throw new AppError(404, 'Проект не найден');

  // Приводим статус к enum ProjectStatus
...
export const emitUserLockStatus = (
  ...
  console.log('📢 emitUserLockStatus called', { userId, updates, ioExists: !!io });
  if (!io) {
    console.warn('⚠️ emitUserLockStatus: io is null, cannot emit');
    return;
  }
...
export const getUsersList = async (params: {
  ...
  const { page, limit, search, role, io } = params;
...
export const toggleBlockUser = async (id: number, currentUserId: number, io: any, logMeta?: any) => {
